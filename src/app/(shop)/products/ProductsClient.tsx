...
},
{
  ...
  const filterProducts = useCallback(() => {
    ...
    if (filters.isOrganic) filtered = filtered.filter(p => p.isOrganic);
...
if (filters.maxPrice !== undefined) {
  filtered = filtered.filter((p) => p.price <= filters.maxPrice!);
    ...
    if (filters.search) filtered = filtered.filter(p => p.title.toLowerCase().includes(filters.search.toLowerCase()));
    ...
    switch (filters.sort) {
      ...
      case 'newest': break; // demo
      case 'popular':
      default: filtered = filtered.sort((a,b) => b.numReviews-a.numReviews); break;
    }
  ...
  const handleClearAll = () => setFilters({
    ...
    isOrganic:false, search:'', sort:'popular', page:1
  });
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="hidden sm:flex items-center bg-white border border-secondary-200 rounded-xl">
                  <button type="button" title="Switch to Grid View" onClick={() => setViewMode('grid')}
                    className={`p-2 rounded-l-xl transition-colors ${viewMode==='grid'?'bg-primary-100 text-primary-600':'text-secondary-400 hover:text-secondary-600'}`}>
                    <Grid className="w-5 h-5" />
                  </button>
                  <button type="button" title="Switch to List View" onClick={() => setViewMode('list')}
                    className={`p-2 rounded-r-xl transition-colors ${viewMode==='list'?'bg-primary-100 text-primary-600':'text-secondary-400 hover:text-secondary-600'}`}>
            ...
            {(filters.category||filters.brand||filters.isOrganic||filters.minPrice||filters.maxPrice) && (
              <div className="flex flex-wrap gap-2 mb-6">
                {filters.category && <span className="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-sm">{filters.category}
                  <button type='button' title='Remove Category Filter' onClick={()=>handleFilterChange({category:''})}><X className="w-4 h-4" /></button>
                </span>}
                {filters.brand && <span className="inline-flex items-center gap-1 px-3 py-1 bg-primary-100 text-primary-700 rounded-full text-sm">{filters.brand}
                  <button type='button' title='Remove Brand Filter' onClick={()=>handleFilterChange({brand:''})}><X className="w-4 h-4" /></button>
                </span>}
                {filters.isOrganic && <span className="inline-flex items-center gap-1 px-3 py-1 bg-green-100 text-green-700 rounded-full text-sm">🌿 Organic
                  <button type='button' title='Remove Organic Filter' onClick={()=>handleFilterChange({isOrganic:false})}><X className="w-4 h-4" /></button>
                </span>}
                {(filters.minPrice || filters.maxPrice) && <span className="inline-flex items-center gap-1 px-3 py-1 bg-accent-100 text-accent-700 rounded-full text-sm">${filters.minPrice||0} - ${filters.maxPrice||'∞'}
                  <button type='button' title='Remove Price Filter' onClick={()=>handleFilterChange({minPrice:undefined,maxPrice:undefined})}><X className="w-4 h-4" /></button>
                </span>}
              </div>
            )}
            ...
            <div className="mt-12 flex justify-center">
              <div className="flex items-center gap-2">
                <button onClick={()=>handlePageChange(filters.page-1)} disabled={filters.page===1} className="px-4 py-2 bg-white border border-secondary-200 rounded-lg text-secondary-600 hover:bg-secondary-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Previous</button>
                {Array.from({length:totalPages}).map((_,i)=>
                  <button key={i} onClick={()=>handlePageChange(i+1)} className={`w-10 h-10 rounded-lg font-medium transition-colors ${filters.page===i+1?'bg-primary-600 text-white':'bg-white border border-secondary-200 text-secondary-600 hover:bg-secondary-50'}`}>{i+1}</button>
                )}
                <button onClick={()=>handlePageChange(filters.page+1)} disabled={filters.page===totalPages} className="px-4 py-2 bg-white border border-secondary-200 rounded-lg text-secondary-600 hover:bg-secondary-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">Next</button>
              </div>
            </div>
          </div>
        </div>
