 ...
 const handleNavigate = (href: string) => {
  ...
  return (
    <div
      ...
      onClick={() => handleNavigate(signin ? `/products/${product.slug}?signin=true` : `/products/${product.slug}`)} // clicking the card navigates too
    >
      {/* Image Container */}
      <div className="relative aspect-square overflow-hidden bg-secondary-100">
        ...
        >
          <button
            onClick={() =>
              handleNavigate(signin ?  `/products/${product.slug}?signin=true`: `/products/${product.slug}`)}
